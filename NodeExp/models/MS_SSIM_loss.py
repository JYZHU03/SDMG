...
def compute_ms_ssim_loss(graph, original_features, reconstructed_features, scales, weights):
    ...
    sim_feature = (reconstructed_features * original_features).sum(dim=-1).pow_(alpha)
    ...
    for i, k in enumerate(scales):
        ...
        orig_agg_feats = F.normalize(orig_agg_feats, p=2, dim=-1)
        recon_agg_feats = F.normalize(recon_agg_feats, p=2, dim=-1)
        sim_k = (recon_agg_feats * orig_agg_feats).sum(dim=-1).pow_(alpha)
        sim_k = (sim_k + 1) / 2
