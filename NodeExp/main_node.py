...
from utils.utils import prepare_graph
...
parser.add_argument("--yaml_dir", type=str, default='./yamls/photo.yaml')
...
def main(cfg):
    ...
    feat = graph.ndata['feat'].to(cfg.DEVICE)
    ...
    edge_index = torch.stack([src, dst], dim=0).to(cfg.DEVICE)
    ...
    for i, seed in enumerate(cfg.seeds):
        ...
        for epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
            ...
            if ((epoch + 1) % 1 == 0) & (epoch > cfg.eva_epoch):
                model.eval()
                acc = node_classification_evaluation(model, cfg.eval_T, graph, feat, adj_pos, num_classes,
